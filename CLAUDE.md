...
- **`CLAUDE.md` と `README.md` の方針を、承認なしに書き換えないこと**。
  技術スタック・スコープ・各種ルールの変更は提案に留め、承認を得てから反映する。
  実装の都合と方針が食い違った場合は、方針を実装に合わせるのではなく、食い違いを報告すること。
  誤字の訂正や、決まっている方針に沿った追記はこの限りではない。
  過去に E2E の実装を変える過程で、技術スタックの定義側が併せて書き換えられた(`b578c07`)。
  方針が実装に合わせて動くと、方針として機能しなくなる
