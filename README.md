...
上の想定は設計の前提として置いた値です。ここから下は**実際に測った値**で、両者を混ぜていません。

### 計測条件

| | |
|---|---|
| マシン | AMD Ryzen 5 5600 (6コア12スレッド) / RAM 32GB / Windows 11 Pro |
| JDK | 21 (Microsoft Build 21.0.9) |
| DB | PostgreSQL 16 (`postgres:16-alpine`, Docker Desktop 29.7.2)。アプリと同一マシン |
| データ | 受注 100,003件 / 明細 300,003行 |
| ステータス分布 | SHIPPED 70% / CANCELLED 10% / CONFIRMED 7% / NEW 5% / SHIPPING_INSTRUCTED 5% / RETURNED 3% |
| 方法 | ウォームアップ5回の後、各パターン20回の応答時間(`curl -w '%{time_total}'`) |
| 計測日 | 2026-09-11 |
...
| パターン | 中央値 | p95 |
|---|---|---|
| `status=NEW`・先頭ページ(オペレーターの主画面) | 20.2ms | 25.0ms |
| `status=NEW`・末尾ページ(249ページ目) | 21.3ms | 23.1ms |
| `mallCode=MALL_A`・先頭ページ | 30.7ms | 32.9ms |
| 絞り込みなし・先頭ページ | 37.9ms | 41.3ms |
| 絞り込みなし・末尾ページ(4999ページ目) | 75.2ms | 80.1ms |

### 実行計画 — インデックスは狙いどおり効いているか

主画面のクエリは、意図した複合インデックスを使っています。10万件でも末尾ページで劣化しません。

```
Limit (actual time=0.093..0.220 rows=20 loops=1)
  ->  Index Scan using idx_orders_status_ordered_at on orders o
        Index Cond: ((status)::text = 'NEW'::text)
        Buffers: shared hit=23
Execution Time: 0.264 ms
```

一方、絞り込みなしの末尾ページはディスクソートに落ちます。

```
Limit (actual time=73.952..73.956 rows=20 loops=1)
  ->  Sort (actual time=61.722..71.271 rows=100000 loops=1)
        Sort Key: ordered_at
        Sort Method: external merge  Disk: 8240kB
        ->  Seq Scan on orders o (rows=100003)
Execution Time: 75.748 ms
```

**この差は設計どおりです。** インデックスは `(status, ordered_at)` の複合で、
「未確認の受注を古い順に処理する」というアクセスパターンに合わせてあります。
絞り込みなしの全件一覧はその形に乗らないため、`ordered_at` で全件を並べ替えることになります。

`ordered_at` 単独のインデックスを足せば解消しますが、**張っていません**。
全件を古い順に末尾ページまで辿る操作は業務上の動線ではなく、
受注を書き込むたびに更新されるインデックスを1本増やす方が割に合わないためです。

### 取込のスループット

`./gradlew perfTest` で計測できます(実行時間が長いため通常の `test` からは除外しています)。

| | 件数 | 所要 | スループット |
|---|---|---|---|
| 新規取込 | 10,000件 | 36.6秒 | 274件/秒 |
| 再取込(全件が冪等キーでスキップ) | 10,000件 | 22.3秒 | 449件/秒 |

1件あたり約3.6msで、速くはありません。**これは「1件ごとに独立したトランザクション」を
選んだ代償です。** バッチ全体を1トランザクションにすればまとめてコミットでき数倍速くなりますが、
1件の異常データで取込済みの全件がロールバックされます。

想定している運用(1分間隔、1日あたり1,000〜5,000件)では1回の取込は数件から数十件で、
274件/秒なら0.1秒以内に収まります。この規模では速度より、
1件の失敗が他の受注を巻き込まないことを優先しています。

なお、この計測にモールAPIのHTTP応答時間とXML/JSONのパースは含まれません。
取込の律速はDB書き込みと冪等キーの制約チェックであり、そこだけを測っています。
