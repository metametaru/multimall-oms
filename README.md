...
このリポジトリは Claude Code(AI)を使って書いています。隠す話ではないので、
何を任せて何を任せていないかを書いておきます。

| | 内容 |
|---|---|
| AI に任せたこと | 実装、テストの記述、調査、リファクタリング、コードレビュー |
| 自分が決めたこと | スコープ、層の構造、業務ルール、設計上のトレードオフ、AIの指摘の採否 |

AIを使うと速度は上がりますが、**方針から静かに外れる**ことがあります。
差分としては小さく、レビューでも見落とします。仕組みで防いでいるのは次の4つです。

**1. 方針は `CLAUDE.md` に書いて渡す**

層の依存方向、テストの書き方、コミットメッセージに何を書くかまで
[CLAUDE.md](CLAUDE.md) に置いています。口頭で都度指示すると、セッションをまたいで揺れます。

**2. 方針そのものをテストにする**

文章で書いた規約は、壊れても気づけません。
「依存方向は presentation → application → domain」「domain はフレームワーク非依存」は
[`LayerRuleTest`](src/test/java/com/minioms/LayerRuleTest.java) が落ちて知らせます。
層の侵食は import 1 行から始まるので、人間の注意力に任せる部分ではありません。

**3. 方針の文書は AI に書き換えさせない**

一度、E2E の実装を変える過程で、技術スタックの定義側が併せて書き換えられたことがあります(`b578c07`)。
実装が方針に合わないとき、方針のほうが動いてしまうと、方針として機能しなくなります。
`CLAUDE.md` と `README.md` の方針変更は提案までとし、承認を経てから反映する、と `CLAUDE.md` に明記しています。

**4. AIのレビューは受けるが、採否は自分が決める**

指摘の内容と、採用・不採用の理由を Pull Request に残しています
(例: [#11](https://github.com/metametaru/multimall-oms/pull/11))。
テストの穴を見つけるのは得意な一方、実行時間や既存テストとの重複といった
このリポジトリ固有の優先順位は判断材料を持っていないので、そこは引き取っています。

AIが書いたコードでも、レビューして通した責任はこちらにあります。
