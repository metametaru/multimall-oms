...
実サーバを起動し、モール取込からログイン・認可・在庫の増減までを一続きに通します。

```
取込(モールA JSON 2件 + モールB XML 1件)
  → ログイン → VIEWER で出荷指示 → 403(業務ロジックに届かない)
  → OPERATOR で確認 → 実6 引2 可4
  → 出荷指示 → 在庫は動かない
  → 出荷完了 → 実4 引0 可4
  → 再取込 → 3件すべてスキップ
```

各層のテストがそれぞれ通っていても、繋いだときに噛み合わないことがあります。
トークンのクレーム名、遷移に対する在庫の反応、冪等キーの効き方は、
通しで動かして初めて確認できるためです。ここでは認証もモールAPIもモックにしません。

ブラウザ操作(Playwright)は使っていません。公開しているのは REST API だけで
操作する画面が無く、画面の無いものをブラウザで動かしても、
確認できるのは結局 HTTP のやり取りだけになるためです。
